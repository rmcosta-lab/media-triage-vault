...
Let the user map each `routing_group` to a destination root, automatically
append the sanitized group name as a subfolder, then generate
...
| Destination path shape | `destination_root / sanitized routing_group / [sanitized country subfolder if enabled] / original_file_name` — no source subfolder tree preserved | Makes the persisted rule a true root while guaranteeing that files from different groups are visibly separated. `collision_policy: error` means the original name travels unchanged, never auto-renamed. |
| What gets sanitized | The generated routing-group and optional country-subfolder segments go through `sanitize_path_component` | `destination_root` is user-supplied and validated for existence/writability directly; the original file name was already valid on the source Windows volume that produced it. The group and country are the path segments generated from classification data, so both must be portable. |
