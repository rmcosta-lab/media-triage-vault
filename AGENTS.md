...
running (`uv run media-organizer serve`) at the URL in
`NEXT_PUBLIC_API_BASE_URL` (`.env.local`, defaults to
`http://127.0.0.1:8000`). The API's CORS policy (`backend/app/api/app.py`)
only allows `localhost`/`127.0.0.1` origins — a browser blocks any
cross-port `fetch`/`EventSource` call otherwise, even though both ends
are local; this only surfaces in a real browser; `TestClient`-based
backend tests never exercise it. `frontend/AGENTS.md`/`CLAUDE.md` are
regenerated by `next dev` on every run (see the file's own header) —
committing the regenerated version is expected, not a stray diff.
