---
name: deep-review
description: Deep multi-agent review of all changes on the current branch — fans out three subagents (correctness, code quality, safety & data integrity) over the branch diff and returns one prioritized findings report. Use when asked for a deep/thorough review of branch changes before merge.
---
...
- Read `specs/mission.md`, `specs/tech-stack.md` and `AGENTS.md` (and, if a matching `specs/<date>-phase-N-<slug>/` directory exists for this branch, its `requirements.md` and `validation.md`) so you can hand the subagents the project's intent, constraints, and this phase's acceptance criteria.
...
- **A — Correctness & bugs**: logic errors, broken behavior, unhandled edge cases, regressions, wrong or missing type annotations, path handling (NFC vs OS-native form, case sensitivity, long paths), encoding, subprocess result parsing, error paths that swallow failures, resource leaks. "Does it actually work, and is anything subtly wrong?"
- **B — Code quality & consistency**: simplification, duplication that should be reused, dead code, naming, module/layer placement (`services/` vs `rules/` vs `repositories/`), whether the change follows the patterns already used in the repo, and test quality — are the failure paths and the phase's edge cases actually covered, or only the happy path? "Is this the simplest, most consistent way to do it?"
- **C — Safety & data integrity**: the mission's non-negotiables. Any outbound network call or non-local dependency; any write, rename, or delete touching source media (forbidden outright before Phase 14, and after it only through the validated plan and journal); collision and overwrite handling; journal correctness, idempotency, and resumability; hash/size validation before a source is removed; `subprocess` invoked with argument lists and never `shell=True`; external tools resolved through the bundled resolver rather than `PATH`; secrets or absolute personal paths committed to the repo. "Can this lose the user's data or leak it off the machine?"
...
- The project intent and non-negotiables from the spec files you read, plus this phase's `requirements.md` decisions if one exists.
- This instruction block so each agent returns findings in a consistent shape:
  ...
  >
  > - **Severity**: `high` (bug / breaks something / violates a non-negotiable / blocks merge), `medium` (should fix), or `low` (nice-to-have / nit).
...
- Any finding that violates a mission non-negotiable is `high` regardless of how the agent rated it.
...
- Use clickable `[scanner.py:42](backend/app/services/scanner.py#L42)` markdown links for every location.
