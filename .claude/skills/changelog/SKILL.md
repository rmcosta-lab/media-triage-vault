...
3. **Formatting rules**:
   ...
   - Each entry is a `- ` bullet with a concise description in English (rewrite raw commit messages into human-readable form if needed).
   - Newest dates first.
   - When a date corresponds to a completed roadmap phase, lead that section with the phase entry (e.g. `- Phase 4 — Scanner: recursive walk with ignore patterns and NFC-normalized paths.`).
